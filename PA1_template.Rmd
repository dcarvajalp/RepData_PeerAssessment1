...
Then, the format of the date is stablished
...
And to replace the missing values with the mean steps by interval:
 ...
 a. We then use the index to acces the mean steps by interval from the summary and we use it to replace the missing values
...
Imputing missing values with mean steps taken per interval does not seem to cause a hugue diference on the calculations of the mean and median steps taken per day:
...
First, on the dataset with the filled-in missing values we create a new column containing the day on which the obervation has been taken
...
We then procede to evaluate the mean steps taken by the time interval for each factor (*weekday* or *weekend*)
